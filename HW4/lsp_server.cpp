...
/*	struct addrinfo hints, *servinfo;
	...
	//	exit(1);
	//}
	//if (!svc_register(transp, LSP_PROGRAM, LSP_VERS, lsp_program_1, IPPROTO_UDP)) {
	...
	new_server->transp=transp;
	sLspServer = new_server;
...
void server_send(lsp_server* server,pckt_type pkt_type,int client_conn_id,int seq_no,const char *payload,int length)
{
	...
	if(pkt_type== DATA_PCKT)
	{
		...
		pkt_cache.seq_no=pkt.seq_no=client_conn->seq_no;
		pkt_cache.data=pkt.data=(char*)payload;
	...
	else if(pkt_type == DATA_ACK)
	{
		...
		pkt_cache.seq_no=pkt.seq_no=seq_no;
		pkt_cache.data="\0";
		pkt.data=s;
	...
	PRINT_PACKET(pkt,"SEND")
	pkt_cache.conn_id=client_conn->conn_id;
	client_conn->packet_cache.putq(pkt_cache);
...
packet * recvfn_1_svc(void * arg, struct svc_req *)
{


}
/*
int * sendfn_1_svc(packet *pkt, struct svc_req *)
{
