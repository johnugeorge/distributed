...
class DataXceiver implements Runnable, FSConstants {
  ...
  public void run() {
    ...
    try {
      ...
      case DataTransferProtocol.OP_WRITE_BLOCK:
        ...
        break;
      case DataTransferProtocol.OP_LOCAL_COPY:
        copyLocalBlock( in );
	break;
  ...
  private void readBlock(DataInputStream in) throws IOException {
    ...
    System.out.println(" In Read Block "+blockId);
  ...
  /**
   * Copy a block locally.
   * 
   * @param in The stream to read from
   * @throws IOException
   */
  private void copyLocalBlock(DataInputStream in) throws IOException {
	    DatanodeInfo srcDataNode = null;
    long srcBlockId=in.readLong();
    long srcStamp= in.readLong();
    Block srcBlock = new Block(srcBlockId, 
        dataXceiverServer.estimateBlockSize, srcStamp);
    System.out.println(" srcBlockId "+srcBlockId);

    long destBlockId=in.readLong();
    long destStamp = in.readLong();
    Block destBlock = new Block(destBlockId, 
        dataXceiverServer.estimateBlockSize, destStamp);
    System.out.println("destBlockId "+destBlockId);
    ...
    DataOutputStream replyOut = null;   // ack Client 
    replyOut = new DataOutputStream(
		    NetUtils.getOutputStream(s, datanode.socketWriteTimeout));

    InputStream blockIn = datanode.data.getBlockInputStream(srcBlock, 0);
    ByteBuffer pktBuf = ByteBuffer.allocate((int)fileSize);
    byte[] buf = pktBuf.array();
    IOUtils.readFully(blockIn, buf, 0,(int)fileSize);
    ...
    DataInputStream checksumIn = new DataInputStream(
    		                    new BufferedInputStream(datanode.data.getMetaDataInputStream(srcBlock),BUFFER_SIZE));
    BlockMetadataHeader header = BlockMetadataHeader.readHeader(checksumIn);


    FSDataset.BlockWriteStreams streams = datanode.data.writeToBlock(destBlock, false,false);
    blockIn = datanode.data.getBlockInputStream(srcBlock, 0);
    OutputStream outStream = streams.dataOut;


    DataOutputStream checksumOut = new DataOutputStream(new BufferedOutputStream(
			    streams.checksumOut, 
			    SMALL_BUFFER_SIZE));
    BlockMetadataHeader.writeHeader(checksumOut, header.getChecksum());
    checksumOut.flush();
    IOUtils.copyBytes(blockIn,outStream,(int)fileSize,(int)fileSize,false);
    outStream.flush();
    ...
    if (targets.length > 0) {
	    ...
	    String mirrorNode = targets[0].getName();
	    mirrorTarget = NetUtils.createSocketAddr(mirrorNode);
	    Socket mirrorSock = datanode.newSocket();
	    try {
		    ...
		    out = new DataOutputStream(
				    new BufferedOutputStream(
					    ...
					    SMALL_BUFFER_SIZE));
		    DataInputStream mirrorIn = new DataInputStream(NetUtils.getInputStream(mirrorSock));

		    out.writeShort(DataTransferProtocol.DATA_TRANSFER_VERSION);
		    out.writeByte(DataTransferProtocol.OP_LOCAL_COPY);
		    out.writeLong(srcBlockId);
		    out.writeLong(srcStamp);
		    out.writeLong(destBlockId);
		    out.writeLong(destStamp);
		    ...
		    if (mirrorInStatus != DataTransferProtocol.OP_STATUS_SUCCESS) {
			    if (mirrorInStatus == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN)
				    throw new IOException("block move failed due to access token error");
			    throw new IOException("block copy is failed");
		    }
		    else
		    {
    ...
    InputStream blockOut=datanode.data.getBlockInputStream(destBlock, 0);
    ByteBuffer pktBuf1 = ByteBuffer.allocate((int)fileSize);
    byte[] buf1 = pktBuf1.array();
    IOUtils.readFully(blockOut, buf1, 0,(int)fileSize);
    System.out.println(" Destination block");
    /*for(byte c : buf1) {
     	    System.out.format("%c ", c);
    }*/
    destBlock.setNumBytes(fileSize);
    datanode.data.finalizeBlock(destBlock);
    datanode.notifyNamenodeReceivedBlock(destBlock, DataNode.EMPTY_DEL_HINT);
    System.out.println("Received block " + destBlock +
		    " of size " + destBlock.getNumBytes());
